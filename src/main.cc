...
int main(int argc, char** argv)
{
  ...
  gettimeofday(&start_time, NULL);
  post_process((float*)outputs[0].buf, height, width, box_conf_threshold, nms_threshold, scale_w, scale_h, out_zps, out_scales, &detect_result_group);
  gettimeofday(&stop_time, NULL);
  ...
  for (int i = 0; i < test_count; ++i) {
    ...
    ret = rknn_outputs_get(ctx, io_num.n_output, outputs, NULL);
// #if PERF_WITH_POST
//     post_process((float*)outputs[0].buf, height, width, box_conf_threshold, nms_threshold, 
